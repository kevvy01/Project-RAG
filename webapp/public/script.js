// --- SCRIPT.JS (MODIFIED FOR NODE.JS) ---

// Ubah ke endpoint internal Node.js, bukan langsung ke n8n
const WEBHOOK_URL = "https://chat.kevvy.my.id/api/chat";
...
function loadSession(id) {
  ...
  session.messages.forEach((msg) => {
    renderBubble(msg.text, msg.sender, false);
  });
  renderSidebar();
}
...
async function kirimPesan() {
  ...
  try {
    // Pemanggilan API ke Node.js Backend
    ...
    // Mengambil jawaban dari n8n (bisa berupa data.output, data.text, atau data.reply tergantung config n8n kamu)
    const reply =
      data.output ||
      data.text ||
      data.reply ||
    ...
    const bubbleLoading = document.getElementById(loadingId);
    if (bubbleLoading)
      bubbleLoading.innerText = "Error: Gagal terhubung ke server Node.js.";
  }
}
...
function renderBubble(text, sender, saveToStorage = true, isLoading = false) {
  ...
  if (saveToStorage && currentSessionId !== null) {
    simpanPesanKeSesi(sender, text);
  }
  return div.id;
}
...
function renderSidebar() {
  ...
  sessions.forEach((session) => {
    ...
    titleBtn.innerText = session.title;
...
function hapusSemuaData() {
  ...
  document.getElementById("confirmMessage").innerText =
    "WARNING: All chat history will be permanently deleted!";
  document.getElementById("btnConfirmDelete").onclick = executeDelete;
  showModal("modalConfirm");
}
